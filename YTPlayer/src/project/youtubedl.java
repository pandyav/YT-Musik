...
class progressbar extends Thread {
	...
	progressbar(double size, String f, JProgressBar jp) {
		this.size = size;
		this.f = f;
		this.jp = jp;
	}
	...
	public void run() {
		...
		while (dsize <= 100) {
			...
			// break;
			// }
			// System.out.println("\t"+(int)dsize+"%");
			jp.setValue((int) dsize);
			percen = (int) dsize + "%";
			try {
				Thread.sleep(100);
			} catch (Exception e) {
...
class youtubedl {
	...
	public void displayIndex(DefaultListModel<String> ds) throws Exception {
		int count = 0;
	...
	public static void download(int dindex, String fname, JProgressBar jp) throws Exception {
		// int avd = Integer.parseInt(index.split("-")[0]);
		String type;
		URL ylink_download;
		// int dindex = Integer.parseInt(index.split("-")[1]);
		// int dindex=Integer.parseInt(index);
		// if(avd==3){
		...
		if (con != null) {
			...
			progressbar t1 = new progressbar(dsize * 1048576, fo, jp);
			t1.start();
			int len1 = 0;
			if (is != null) {
	...
	// URL("https://www.youtube.com/watch?v=cOsqdghHcIk"));
	// // youtubedl obj = new youtubedl(new
	...
	// URL("file:///Users/Vaib/Desktop/Kalimba.mp3"));
	////
